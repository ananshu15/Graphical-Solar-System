...
ID 300362622
Class: CSCI 172
Project name: main05.cpp


MODELS:
There should be a total of 6 models that have been loaded.
    ateneam-2
    Cow
    Cube
    Elephant
...
CONTROLES:
Using the WASD keys to move forwards, left, back, and right.
The directions are static in the program
Left and Right Arrow keys will rotate all models present here.

Collision detection has also been implemented.
You cannot go inside of models limits are applied.
You cannot exit the sky box limits are applied.
...
//variable used inside of loadTexture
GLuint tex;
//Global Variables to help us rotate and zoom
float rotX, rotY, rotCamX, rotCamY, moveX, moveY, zoom = 0;
float prev_x, prev_y = 0.0;
/* GLUT callback Handlers */


//contain the texture coordinates
class v2{
    public:
    float a2;
    float b2;
...
//vectors that are used for triangles
class v3{
    public:
    float a3;
    float b3;
...
//Faces with some vertexes that will be used
...
//contains vertexes and textures for triangles
...
//contains vertexes and textures for Quads
...
//contains Vertexes and textures for Pentagons
...
// contains Vertexes and textures for Hex
...
//contains vertexes, textures,for Triangles
...
//contains vertexes, textures, and normals for Quads
...
//  contains vertexes, textures, and normals for Pentagon
...
//contains vertexes, textures, and normals for Hexagon
class faceVertexTextureNorms6{
   public:
    float v1, v2, v3, v4, v5, v6, t1, t2, t3, t4, t5, t6, n1, n2, n3, n4, n5, n6;   //Vertexes used
...
v3 subtractVertexes(v3 ver1, v3 ver2) //subtracting vertex values{
...
v3 crossProduct(v3 ver1, v3 ver2) //cross product value {
    v3 crossP = v3(0, 0, 0);
...
class Model{
    public:
    //Vertexes
    vector<faceOnlyVertex> faceVertexes;

    //Triangles
    vector<faceVertexTexture3> faceVertTexTri;
    vector<faceVertexTexture3> faceVertNormTri;
    vector<faceVertexTextureNorms3> faceVertTexNormTri;
    ...
    vector<v2> v2TexCoords;   //For 2D texture
    vector<v3> v3Vectors;     //Triangle Vertexes
    vector<v3> v3Normals;     //Triangle Normals

    vector<v4> v4Vertexes;    //Quad Vertexes
    vector<v4> v4Normals;    //Quad Normals
    ...
    void readFile(){
        string line;
        vector<string> lines;   //represent all the lines

        ifstream in(fileName);  //Taking in the file and read it
        if(!in.is_open())       //If we are not able to find it or load it, print the error and return
        {
            cout << "Cannot Load Model " << fileName << endl;
            return;
        }
        while(!in.eof())        //If we are able to open it
        {
            getline(in, line);
            lines.push_back(line);  //Placing into the vector
        }
        in.close(); //Closing the file.

        //This is where we will fill in our vectors
        float a, b, c, ver1, ver2, ver3, ver4, ver5, ver6, tex1, tex2, tex3, tex4, tex5, tex6, norm1, norm2, norm3, norm4, norm5, norm6;  //We will use these for filling in the data piece by piece. We will be working with v3 vectors
        ...
        for(string &line : lines)
        {
            ...
            if(line[0] == 'v')
            {
                if(line[1] == ' ')  //If the line is a vector
                {
                    sscanf(line.c_str(), "v %f %f %f", &a, &b, &c); // float for a, b, and c
                    v3Vectors.push_back(v3(a, b, c));
                }
                else if(line[1] == 't')  //If the line is a texture coordinate
                {
                    sscanf(line.c_str(), "vt %f %f", &a, &b); // float for a, b, and c
                    v2TexCoords.push_back(v2(a, b)); //Adding a new v2 variable
                }
                else
                {
                    sscanf(line.c_str(), "vn %f %f %f", &a, &b, &c); // float for a, b, and c
                    v3Normals.push_back(v3(a, b, c));   //Adding new normal coordinates
                }
            }
            else if(line[0] == 'f')
            {
                ...
                //Only vertexes and Textures for Triangles
                ...
                //Only vertexes and textures for Quads
                ...
                //Only vertexes and textures for Pentagons
                ...
                //All 3 Vertexes, textures, and normals
                else if(numOfSlash == 6){
                    //This will check if we have texture coordinates or not
                ...
                //All 3 Vertexes, textures, and normals for Quads
                else if(numOfSlash == 8){
                    //This will check if we have texture have coordinates or not
                    if((line.find('/', line.find('/')+1)) - line.find('/') == 1){
                ...
                //All 3 Vertexes, textures, and normals for Pentagons
                ...
                //All 3 Vertexes, textures, and normals for Hexagons
    ...
    // add normal values to our faces
    void faceNormalizeTriangles(){  // only used for triangles
        if(v3Normals.size() == 0){  //Check if there are any normals, if not then we can add them, otherwise ignore
    ...
    //used inside of the display function.
    void drawModel(){
        //Draw all vertexes
        for(int i = 0; i < faceVertexes.size();i++){
            if(!WireFrame)                  //for the Wire frame
        ...
        for(int i = 0; i < faceVertTexTri.size();i++){  //drawing the TRIANGLES containing the vertex
            if(!WireFrame)
            glBegin(GL_TRIANGLES);
            else
            glBegin(GL_LINE_LOOP);
        ...
        for(int i = 0; i < faceVertNormTri.size();i++){ //drawing the TRIANGLES containing the vertex
            if(!WireFrame)
            glBegin(GL_TRIANGLES);
            else
            glBegin(GL_LINE_LOOP);
        ...
        for(int i = 0; i < faceVertTexNormTri.size(); i++){ //drawing the TRIANGLES containing the vertex
            if(!WireFrame)
            glBegin(GL_TRIANGLES);
            else
            glBegin(GL_LINE_LOOP);
        ...
        // Quads
        for(int i = 0; i < faceVertTexQuad.size();i++){  //drawing the TRIANGLES containing the vertex
            if(!WireFrame)
            glBegin(GL_QUADS);
            else
            glBegin(GL_LINE_LOOP);
        ...
        for(int i = 0; i < faceVertNormQuad.size(); i++){ //drawing the TRIANGLES containing the vertex
            if(!WireFrame)
            glBegin(GL_QUADS);
            else
            glBegin(GL_LINE_LOOP);
        ...
        for(int i = 0; i < faceVertTexNormQuad.size();i++){ //drawing the TRIANGLES containing the vertex
            if(!WireFrame)
            glBegin(GL_QUADS);
            else
            glBegin(GL_LINE_LOOP);
        ...
        //Drawing Pentagons
        for(int i = 0; i < faceVertTexPent.size();i++){  //drawing the TRIANGLES containing the vertex
            if(!WireFrame)
            glBegin(GL_POLYGON);
            else
            glBegin(GL_LINE_LOOP);
        ...
        for(int i = 0; i < faceVertNormPent.size(); i++){   //drawing the PENTAGONS
            if(!WireFrame)
            glBegin(GL_POLYGON);
            else
            glBegin(GL_LINE_LOOP);
        ...
        for(int i = 0; i < faceVertTexNormPent.size();i++){ //drawing the PENTAGONS
            if(!WireFrame)
            glBegin(GL_POLYGON);
            else
            glBegin(GL_LINE_LOOP);
        ...
        //Drawing Hexagons
        for(int i = 0; i < faceVertTexHex.size();i++){   // drawing the HEXAGONS
            if(!WireFrame)
            glBegin(GL_POLYGON);
            else
            glBegin(GL_LINE_LOOP);
        ...
        for(int i = 0; i < faceVertNormHex.size(); i++){  //drawing the HEXAGONS
            if(!WireFrame)
            glBegin(GL_POLYGON);
            else
            glBegin(GL_LINE_LOOP);
        ...
        for(int i = 0; i < faceVertTexNormHex.size();i++){ //drawing the HEXAGONS
            if(!WireFrame)
            glBegin(GL_POLYGON);
            else
            glBegin(GL_LINE_LOOP);
...
static void resize(int width, int height)
{
    ...
    glLoadIdentity();
	gluPerspective (50.0f,1,0.1f, 500.0f); //Changed to 200.0f for more zoom in
 }
 ...
 void loadTexture(char *fileName){
    ...
    glBindTexture(GL_TEXTURE_2D, tex); // images are 2D arrays of pixels
    int width, height;                 // width & height for image reader
    unsigned char* image;
    image = SOIL_load_image(fileName, &width, &height, 0, SOIL_LOAD_RGB);
    if(!image)  //If there is no image or it cannot find it, print an error
...
static void display(void)
{
    ...
    if(WireFrame)
		glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);		//Drawing Wireframe
	else
    ...
    //skybox //
     glPushMatrix();
        glScalef(50.0, 50.0, 50.0); //Make it 50 times bigger on all sides
        glDisable(GL_NORMALIZE);    //Disable normalize
        glDisable(GL_LIGHTING);     //Disable lighting

        loadTexture("images/brick.jpg");
        glBegin(GL_QUADS);  //GL_QUADS allows us to create a 4 sided polygon that we can use
            glTexCoord2f(1.0, 0.0);     //This is the First point (x, y)
            glVertex3f(-1.0, 1.0, -1.0);    //These specify the vertexe

            glTexCoord2f(1.0, 1.0);     //This is the Second point
        ...
        loadTexture("images/brick.jpg");
        glBegin(GL_QUADS);                  //Second Square to be made
            ...
            glTexCoord2f(0.0, 1.0);     //First point that moves behind us
            glVertex3f(1.0, -1.0, 1.0);
        ...
        loadTexture("images/brick.jpg");
        glBegin(GL_QUADS);                  //Third Square will be made
            glTexCoord2f(0.0, 0.0);         //texCoord
            glVertex3f(-1.0, 1.0, -1.0);
        ...
        glBegin(GL_QUADS);                  //This will also complete an entire 360 degree
            glTexCoord2f(0.0, 0.0);         //This is also being created opposite to the front
            glVertex3f(-1.0, 1.0, 1.0);
        ...
        loadTexture("images/roof.jpg");
        glBegin(GL_QUADS);             //Fifth square made
            glTexCoord2f(0.0, 1.0);
            glVertex3f(1.0, 1.0, -1.0);
        ...
        loadTexture("images/floor.jpg");
        glBegin(GL_QUADS);              //Last square made
            glTexCoord2f(1.0, 1.0);
            glVertex3f(-1, -0.01, 1);
...
static void key(unsigned char key, int x, int y)
{
    switch (key)
    {
        ...
        case 'w':   //Forward
            ...
            //Don't allow movement towards the trunk model
        ...
        case 'a':   //Left
            if(moveX < 40)
                moveX += 1;
            //Don't allow movement towards the ate model
            if(zoom > 5 && zoom < 13 && moveX < 2 && moveX > -2){
                moveX -= 1;
            }
            //Don't allow movement towards the elephant model
...
// mouse movement and coordination
void mouseMove(int x,int y)
{
    //If the old y is less than the new one, we will rotateX by 5
     if(prev_y < y){
        if(rotCamX <= 15){
            rotCamX += 1.5;
        }
     }
     //If the old y is greater than the new one, we will rotateX by -5
      if(prev_y > y){
        if(rotCamX >= -80){
            rotCamX -= 1.5;
        }
     }
     //If the old x is less than the new one, we will rotateY by 5
     if(prev_x < x){
        rotCamY += 3;
     }
     //If the old x is greater than the new one, we will rotateY by -5
      if(prev_x > x){
        rotCamY -= 3;
     }
    //Updating our mouse positions
    prev_x = x;
    prev_y = y;
}
...
int main(int argc, char *argv[])
{
    ...
    cow.readFile(); //reading the files first
    ele.readFile();
    stat.readFile();
